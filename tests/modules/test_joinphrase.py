...
class TestJoinphrase():
    ...
    def testJoinphrase(self):
        ...
        assert len(self.connection.getRoomByID("testroom2").joinphrases) == 0
        ...
        assert len(self.connection.getRoomByID("testroom2").joinphrases) == 1
        assert self.connection.getRoomByID("testroom2").joinphrases["testuser"] == "Join pHRase &&||"
        ...
        message = dummies.DummyMessage(
            ...
            sender=dummies.DummyUser(isAdmin=True),
            room=self.connection.getRoomByID("testroom2")
        ) # Adding in a room
        self.module.addJP(message)
        assert len(self.connection.getRoomByID("testroom2").joinphrases) == 1
        assert self.connection.getRoomByID("testroom2").joinphrases["testuser"] == "Join pHRase &&||"
